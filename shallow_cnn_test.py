...
weights = {
	...
	'out': tf.Variable(tf.random_normal([sen_dim*3*2, n_classes],dtype=tf.float32))
}
...
x1_min = tf.reduce_min(x1, 2)
x2_min = tf.reduce_min(x2, 2)

x1_mean = tf.reduce_mean(x1, 2)
x2_mean = tf.reduce_mean(x2, 2)

x1_max = tf.reduce_max(x1, 2)
x2_max = tf.reduce_max(x2, 2)

x12 = tf.concat(1, [x1_min, x1_mean, x1_max])
x22 = tf.concat(1, [x2_min, x2_mean, x2_max])
